  ...
  def sort_param(sortable_name, *args)
    ...
    unless session[:sortable_column_headers].nil? || session[:sortable_column_headers][sorted_key].nil?
      ...
      if sch_key = self.sortable_column_header_data[sortable_key]
        # BERKMAN FIXME - Make this work for MySQL
        if session[:sortable_column_headers][sorted_key].include?(sch_key+' ASC NULLS FIRST')
          if session[:sortable_column_headers][sorted_key].index(sch_key+' ASC NULLS FIRST') == 0
            sort_param = 'sortdesc'
          end
        # BERKMAN FIXME - Make this work for MySQL
        elsif session[:sortable_column_headers][sorted_key].include?(sch_key+' DESC NULLS LAST')
          if session[:sortable_column_headers][sorted_key].index(sch_key+' DESC NULLS LAST') > 0
            sort_param = 'sortdesc'
          end
        end
      end
...
private
  ...
  def store_sort(sortable_name, data_sort_param, sortable_key, sch_data)
    if sch_data.include? sortable_key
      if data_sort_param
        # BERKMAN FIXME - Make this work for MySQL
        sort_sql     = sch_data[sortable_key]+' ASC NULLS FIRST'
        sort_sql_not = sch_data[sortable_key]+' DESC NULLS LAST'
      else
        # BERKMAN FIXME - Make this work for MySQL
        sort_sql     = sch_data[sortable_key]+' DESC NULLS LAST'
        sort_sql_not = sch_data[sortable_key]+' ASC NULLS FIRST'
      end
